...
- (void)drawRect:(CGRect)rect
{
    ...
    dayFont = [UIFont fontWithName:@"AvenirNext-Regular" size:8];
    ...
    monthFont = [UIFont fontWithName:@"AvenirNext-Regular" size:16];
    for (int i = 0; i < dayTitles.count; i++) 
    {
        NSInteger index = i + (_mondayFirstDayOfWeek?1:0);
        index = index % 7;
        NSString *dayTitle = [[dayTitles objectAtIndex:index] uppercaseString];
        ...
        if(dayFont) sz = [dayTitle sizeWithAttributes:@{NSFontAttributeName:dayFont}];
        
        CGRect dayHeaderFrame = CGRectMake(floor(i * hDiff) - 1
                                           , headerHeight + (kPMThemeDayTitlesInHeaderIntOffset * vDiff - sz.height) / 2+3.5
                                           , hDiff
                                           , sz.height);
    ...
    CGRect textFrame = CGRectMake(0
                                  , (headerHeight - monthTitleSize.height) / 2+7
                                  , width
                                  , monthFont.pointSize);
    ...
    BOOL showsLeftArrow = NO;
    BOOL showsRightArrow = NO;
...
- (void)drawRect:(CGRect)rect
{
    CGContextRef context       = UIGraphicsGetCurrentContext();
    UIFont *calendarFont       = kPMThemeDefaultFont;
    calendarFont = [UIFont fontWithName:@"AvenirNext-Regular" size:16];
    UIEdgeInsets shadowPadding = kPMThemeShadowPadding;
    CGFloat headerHeight       = kPMThemeHeaderHeight;
    ...
    for(int i = 0; i <= weekdayOfFirst-2; i++) 
    {
        ...
        NSString *string = [NSString stringWithFormat:@"%02d", day];
    ...
    int startIndex = (int)[[self.calendarView.allowedPeriod.startDate dateWithoutTime] daysSinceDate:monthStartDate] + weekdayOfFirst - 1;
    int endIndex   = (int)[[self.calendarView.allowedPeriod.endDate dateWithoutTime] daysSinceDate:monthStartDate] + weekdayOfFirst - 1;
