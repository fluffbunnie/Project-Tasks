...
-(UIBarButtonItem *)checkoutButton {
    if (_checkoutButton == nil) {
        ...
        [checkoutBt setTitle:@"Request stay" forState:UIControlStateNormal];
...
/**
 * Send a new message with a given context and type
 * @param NSString
 * @param NSString
 */
-(void)sendMessage:(NSString *)message withType:(NSString *)type {
    [self.chatMessages sendMessageWithText:message andType:type];
}
...
-(void)bookTripWithWithStartDate:(NSString *)startDate endDate:(NSString *)endDate numberOfGuests:(int)numGuests reason:(NSString *)reason andPlace:(PFObject *)propertyObj {
    ...
    tripObj[@"reason"] = reason;
    tripObj[@"approval"] = @"NO";
    
    [tripObj saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        [self.loadingView dismiss];
        if (!error && succeeded) {
            [self.chatMessages sendMessageWithText:tripObj.objectId andType:MESSAGE_TYPE_BOOK];
            
            NSString *about = self.userObj[@"description"] ? self.userObj[@"description"] : @"";
            NSString *association = self.userObj[@"association"] ? self.userObj[@"association"] : @"";
            NSString * favoritePlace = self.userObj[@"favoritePlace"] ? self.userObj[@"favoritePlace"] : @"";
            NSString * dreamPlace = self.userObj[@"dreamPlace"] ? self.userObj[@"dreamPlace"] : @"";
            
            if (about.length == 0 || association.length == 0 || favoritePlace.length == 0 || dreamPlace.length == 0) {
                MyProfileNudgeViewController *myProfileNudge = [[MyProfileNudgeViewController alloc] init];
                myProfileNudge.capturedBackground = [Device captureScreenshot];
                myProfileNudge.userObj = self.userObj;
                [self.navigationController pushViewController:myProfileNudge animated:NO];
            }
...
/**
 * Show the trip popup from the deep link
 */
-(void)showTripPopup {
    if (self.tripObj != nil) {
        ...
        if ([self.userObj.objectId isEqualToString:guestObj.objectId]) {
            ...
            myStayRequestViewController.tripObj = self.tripObj;
            ...
            incomingStayRequestViewController.tripObj = self.tripObj;
        ...
        if ([message.messageType isEqualToString:MESSAGE_TYPE_BOOK] || [message.messageType isEqualToString:MESSAGE_TYPE_CANCEL]) {
            ...
            if (isFromUser) {
                [tripQuery getObjectInBackgroundWithId:message.tripObjId block:^(PFObject *object, NSError *error) {
                    if (!error && object != nil) {
                        MyStayRequestViewController *myStayRequestViewController = [[MyStayRequestViewController alloc] init];
                        myStayRequestViewController.tripObj = object;
                        myStayRequestViewController.capturedBackground = [Device captureScreenshot];
                        [self.navigationController pushViewController:myStayRequestViewController animated:NO];
                    }
                }];
                ...
                [tripQuery getObjectInBackgroundWithId:message.tripObjId block:^(PFObject *object, NSError *error) {
                    if (!error && object != nil) {
                        IncomingStayRequestViewController *incomingStayRequestViewController = [[IncomingStayRequestViewController alloc] init];
                        incomingStayRequestViewController.tripObj = object;
                        incomingStayRequestViewController.capturedBackground = [Device captureScreenshot];
                        [self.navigationController pushViewController:incomingStayRequestViewController animated:NO];
                    }
                }];
