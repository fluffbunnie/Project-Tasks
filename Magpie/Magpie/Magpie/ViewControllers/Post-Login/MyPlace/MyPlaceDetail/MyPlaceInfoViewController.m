...
static NSString *LISTING_TYPE_TITLE = @"What space do you have to offer?";
...
static NSString *ROOM_TITLE = @"How big is your place?";
...
-(FloatPlaceholderTextView *)houseDescription {
    if (_houseDescription == nil) {
        ...
        if (self.propertyObj[@"fullDescription"]) {
            ...
            CGSize newSize = [_houseDescription sizeThatFits:CGSizeMake(self.screenWidth - 40, MAXFLOAT)];
...
- (void)textViewDidChange:(UITextView *)textView {
    ...
    self.propertyObj[@"fullDescription"] = (textView.text.length > 0) ? textView.text : @"";
}
...
-(void)goBack {
    if (self.shouldChange) {
        [ToastView showToastInParentView:self.windowView withText:@"Saved" withDuaration:1.5];
