...
-(void)propertyRequestResult:(Property *)property {
    if (property == nil) [self showErrorMessage];
        ...
        propertyObj[@"state"] = PROPERTY_APPROVAL_STATE_PENDING;
...
-(void)completeButtonClicked {
    ...
    NSInteger currentViewControllerIndex = [self.navigationController.viewControllers indexOfObject:self];
    MyPlaceListViewController *myPlaceListViewController = [self.navigationController.viewControllers objectAtIndex:currentViewControllerIndex - 2];
    [self.navigationController popToViewController:myPlaceListViewController animated:NO];
}
