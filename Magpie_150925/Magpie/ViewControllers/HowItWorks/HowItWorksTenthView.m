...
@implementation HowItWorksTenthView

#pragma mark - initiation screen 1
/**
 * Lazily init the value background image
 * @return UIImageView
 */
-(UIImageView *)valueBackgroundImageView {
    if (_backgroundImageView == nil) {
        ...
        _backgroundImageView.image = [UIImage imageNamed:VALUE_BACKGROUND_SCREEN_4];
    }
...
-(void)testinomialsButtonClick {
    [_howItWorksViewDelegate closeHowItWorks];
}
...
-(id)init {
    ...
    _animationStatus = 0;
    self = [super initWithFrame:CGRectMake(self.viewWidth * 9, 0, self.viewWidth, self.viewHeight)];
    if (self) {
...
-(void)stopAnmationAndShowView {
    ...
    self.valueLabelText.alpha = 1;
    self.valueMagpieLogoImage.alpha = 1;
    self.valueTestinomialsButton.alpha = 1;
}
