...
static NSString * const HOW_IT_WORKS_BUTTON_TEXT = @"Sneak peek at how Magpie works";
...
@property (nonatomic, strong) UIButton *sneakPeakButton;
...
/**
 * Lazily init the sneak peek button
 * @return UIButton
 */
-(UIButton *)sneakPeakButton {
    if (_sneakPeakButton == nil) {
        _sneakPeakButton = [[UIButton alloc] initWithFrame:CGRectMake((self.viewWidth - 300)/2, self.viewHeight - 100, 300, 50)];
        _sneakPeakButton.layer.cornerRadius = 25;
        _sneakPeakButton.layer.masksToBounds = YES;
        _sneakPeakButton.titleLabel.font = [UIFont fontWithName:@"AvenirNext-Medium" size:15];

        [_sneakPeakButton setTitle:HOW_IT_WORKS_BUTTON_TEXT forState:UIControlStateNormal];

        [_sneakPeakButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];

        [_sneakPeakButton setBackgroundImage:[FontColor imageWithColor:[FontColor greenThemeColor]] forState:UIControlStateNormal];
        [_sneakPeakButton setBackgroundImage:[FontColor imageWithColor:[FontColor darkGreenThemeColor]] forState:UIControlStateHighlighted];

        [_sneakPeakButton addTarget:self action:@selector(howItWorksButtonClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _sneakPeakButton;
}
...
-(id)init {
    ...
    if (self) {
        ...
        [self addSubview:[self sneakPeakButton]];
...
/**
 * Handle the behavior when user click on the sneak peek button
 */
-(void)howItWorksButtonClick {
    [self.delegate sneakPeekAtHowItWorks];
}
